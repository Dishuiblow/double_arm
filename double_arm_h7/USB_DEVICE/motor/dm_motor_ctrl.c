...
void dm_motor_init(void)
{
	...
	motor[Motor1].ctrl.pos_set 	= 0.28f;
...
void fdcan1_rx_callback(void)
{
	...
	switch (rec_id)
	{
 		 case 0x11: 
			...
			for(int i=0; i<8; i++)
			{
				motor4340[Motor1].ori_date[i] = rx_data[i];
			}
			init_motor_data[0] = 0;
		break;
		case 0x12: 
			...
			for(int i=0; i<8; i++)
			{
				motor4340[Motor2].ori_date[i] = rx_data[i];
			}
			init_motor_data[1] = 0;
		break;
		case 0x13: 
			...
			for(int i=0; i<8; i++)
			{
				motor4340[Motor3].ori_date[i] = rx_data[i];
			}
			init_motor_data[2] = 0;
		break;
		case 0x14: 
...
/**
 * @brief CAN2负责接收右臂电机反馈数据，右臂电机仍设为1-6，但在代码中映射到8-13，减少上位机修改量
 * 
 */
...
void fdcan2_rx_callback(void)
{
	...
	switch (rec_id2)
	{
		...
		case 0x12:
			...
			break;
		case 0x13:
			for(int i=0; i<8; i++) { motor4340[Motor10].ori_date[i] = rx_data2[i]; }
			break;
 		case 0x14: 
		...
		case 0x15: 
			...
			break;
		case 0x16: 
			for(int i=0; i<8; i++) { motor[Motor13].ori_date[i] = rx_data2[i]; }
			break;
	}
}
